...
def plot_file(matFile_, fig_):
    ...
    if Title_:
        ...
        opis_[fext_]['title'] =''
    gr_name_ = material_[4:] + '_' + ptkl_ + '_' + fext_[1:]
    if fext_ in ['.23','.23p', '.eb','.stp','.awe', '.xer', '.528']:
        ...
        if fext_ in ['.awe', '.xer', '.eb', '.stp', '.528']:
            fig_ += 1
            xxp.xgraf_plot(xx_, yy_, ptkl_ + fext_[0:], opis_[fext_] )
            gsave(gr_name_)
            if fext_ in ['.stp'] and False:
                fig_ += 1
                opis_[fext_]['ylabel'] = r'$L, см$'
                opis_[fext_]['xlabel'] = r'$\varepsilon,кэВ$'

                opis_[fext_]['bxlog'] = False
                Ro = 2.7
                xxp.xgraf_plot(xx_/10**3, yy_/Ro, fig_, opis_[fext_] )
                pass
...
##                u'Возбуждение атома', u'Электроионизация', u'Аннигиляция', )
            ...
            xxp.xgraf_plot(xx_, xy_, fig_, opis_['len'] )
            gsave(gr_name_ + '_lenth')

            fig_ += 1
            ip_ = yy_<10**(-15)
            yy_[ip_] = 0.
            xxp.xplot_cs(xx_, yy_, fig_, opis_[fext_])
            gsave(gr_name_)
            opis_[fext_]['ylabel'] = r'$см^{-1}$'
            xxp.xplot_cs(xx_, (yy_*Ro), ptkl_+fext_, opis_[fext_])
...
##                    pass
                ...
                dfm_[i,:] = df_
            fig_ +=1
            xxp.xtable_polar(ee_, xt_, dfm_, fig_)
            gsave(gr_name_ + "_directrissa")
            fig_ +=1
            xxp.xgraf_plot(ee_, cav_, fig_, opis_[fext_] )
...
##                    Eph_ = e_ / (1. + (1. - yy_) * e_/phis.E0 )
            ...
            if fext_ in ['.cv'] :
                fig_ +=1
                xxp.xtable_plot(ee_, xt_, Df_,fig_, bxlog = False, bylog = False,
...
##                                ylabel = r'$\varepsilon_{\gamma}$', xlabel = r'$\gamma$' )
            ...
            fig_ += 1
            if fext_ in ['.527']:
                xxp.xtable_plot(ee_, gg_, yy_, fig_,

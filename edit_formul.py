...
class Example(Frame):
    ...
    def __init__(self, parent, db):
        ...
        (self._na, self._nu) = cor.read_elements(sd)  # na[nu['Cu']]
##        self._nu = nu
    ...
    def initUI(self):
        ...
        Label(frame_, text='Кол-во атомов').grid(row=i, column=1)
        Label(frame_, text='Атомная масса').grid(row=i, column=2)
        Label(frame_, text='Массовая Доля').grid(row=i, column=3)
        k = i + 1

        for i in range(self._n):
            bt = Button(frame_, text='  ',  relief=GROOVE)
            bt.bind('<Button-1>', self.tblMendel)
            bt.grid(row=i+k, column=0, padx=5, pady=1, sticky=W+E)
            self._bt.append(bt)
            self._pv[i].set(0.0)
            ent = Entry(frame_, textvariable=self._pv[i], relief=SUNKEN,
                    justify=RIGHT)
            ent.grid(row=i+k, column=3, padx=5, pady=1, sticky=W+E)
            self._ent.append(ent)
            pass
        bs = Button(frame_, text='Сохранить', command=self.onSave, relief=RAISED).grid(row=i+k,
                column=0, columnspan=2, padx=5, pady=10, sticky=W+E)
...
##        print(' k %d'%k)
    ...
    def tblMendel(self, event):
        ...
        for k in range(1, self._nelm):
            i,j=z2rc(k)
            b = Button(self._ta, text=str(k) + ':' + self._dmt[k]['name'], relief=RIDGE,command=(lambda k=k:self.elem(k, event)))
